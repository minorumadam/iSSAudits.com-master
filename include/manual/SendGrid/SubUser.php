...
class SubUser {
	...
	public function __construct ($user, $password, $email, $domain, $firstName = '-',
								 ...
								 $state = '-', $zip = '-', $country = '-',
								 $phone = '-', $website = '-', SubUserApi $api = null) {
		$this->username = $user;
	...
	public function setEventPostUrl ($app, array $settings) {
		$this->api->debug("Configuring app '$app' for '{$this->username}'");
		$params = array('task' => 'set', 'name' => $app);
		$this->execute('customer.eventposturl.json', $params, $settings);
	}
	/**
	 ...
	 */
	public function attachApi (SubUserApi $api) {
		$this->api = $api;
	}
